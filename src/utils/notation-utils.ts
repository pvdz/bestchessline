...
export const rawMoveToSAN = (rawMove: string, fen: string): string => {
  ...
  const toFile: number = fileToIndex(to[0]);
  const toRank: number = rankToIndex(to[1]);
  ...
  const destPiece: string | null = board[toRank][toFile];
 ...
 */
export function sanToLongNotation(sanMove: string, fen: string): string | null {
  try {
 ...
 */
export function convertLineToLongNotation(
  sanLine: string,
  startingFEN: string,
): string {
 ...
 */
export function convertSANLineToLongNotation(
  sanMoves: string[],
  startingFEN: string,
): string {
  let currentFEN = startingFEN;
  const longMoves: string[] = [];
  let moveNumber = 1;
  let whiteMove = "";
 ...
 */
export function getPieceCapitalized(piece: string): string {
  const pieceMap: { [key: string]: string } = {
    P: "P",
    p: "P", // Pawn
    N: "N",
