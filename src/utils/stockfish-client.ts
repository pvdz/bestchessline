...
import { assertMoveExpensive, analyzeMove } from "./move-validator.js";
...
import { applyLongMoveToFEN, applyMoveToFEN } from "./fen-manipulation.js";
...
function createStockfishFallbackWorker(): void {
  try {
    ...
    showErrorToast(
      "Failed to initialize Stockfish fallback engine: " +
...
function parseInfoMessage(message: string): void {
  ...
  for (let idx = 0; idx < pv.length; idx++) {
    ...
    if (idx === 0) {
      ...
      if (isWhitePiece !== shouldBeWhite) {
        console.warn(
          "stockfish-info: PV[0] does not match side-to-move; skipping info line",
          {
            fen: validateFen,
            moveStr,
            piece: move.piece,
    ...
    if (
      (move.from === "e1" && move.to === "g1" && move.piece === "K") ||
    ...
    if (!vr.isValid) {
      console.error(
        "stockfish-info: HARD-STOP — validator rejected engine PV move",
        {
          fen: validateFen,
          moveStr,
          index: idx,
  ...
  if (pv.length !== pvMoves.length) {
    console.warn(
      "stockfish-info: PV length mismatch",
      [message],
      [pv],
      [pvMoves],
      {
  ...
  if (!firstMove) {
    console.warn(
      "stockfish-info: dropping PV because no valid moves remained",
      [message],
      [pv],
      {
        fen,
  ...
  ASSERT(
    ...
    "stockfish-info: analysisMove.move missing",
    { analysisMove },
  );
