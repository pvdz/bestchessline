import type {
  ...
  SerializedTreeDiggerNode,
  TreeDiggerStateMetadata,
} from "../types.js";
 ...
 */
const serializeTreeNode = (node: TreeDiggerNode): SerializedTreeDiggerNode => {
  return {
    fen: node.fen,
    move: node.move,
    score: node.score,
 ...
 */
const deserializeTreeNode = (
  serializedNode: SerializedTreeDiggerNode,
): TreeDiggerNode => {
  return {
 ...
 */
const calculateAnalysisStatistics = (
  analysis: TreeDiggerAnalysis,
): TreeDiggerStateMetadata["statistics"] => {
  ...
  const maxDepth = Math.max(
    ...analysis.nodes.map((node) => {
      const getMaxDepth = (n: TreeDiggerNode, depth: number): number => {
 ...
 */
export const serializeTreeDiggerState = (
  ...
  const metadata: TreeDiggerStateMetadata = {
 ...
 */
export const validateTreeDiggerState = (
  ...
  const versionCompatible =
    stateExport.metadata.version === TREE_DIGGER_STATE_VERSION;
  if (!versionCompatible) {
    warnings.push(
      `State version ${stateExport.metadata.version} may not be compatible with current version ${TREE_DIGGER_STATE_VERSION}`,
    );
  }

  // Board position check
  const boardPositionMatch =
    stateExport.metadata.boardPosition === currentBoardPosition;
  ...
  const configMatch =
    JSON.stringify(stateExport.metadata.configuration) ===
 ...
 */
export const deserializeTreeDiggerState = (
  stateExport: TreeDiggerStateExport,
): { analysis: TreeDiggerAnalysis; state: TreeDiggerState } => {
  ...
  const analysis: TreeDiggerAnalysis = {
  ...
  const reconstructParents = (
    nodes: TreeDiggerNode[],
    parent: TreeDiggerNode | null = null,
  ): void => {
 ...
 */
export const exportTreeDiggerState = (
  analysis: TreeDiggerAnalysis | null,
  state: TreeDiggerState,
): void => {
  try {
 ...
 */
export const copyTreeDiggerStateToClipboard = async (
  analysis: TreeDiggerAnalysis | null,
  state: TreeDiggerState,
): Promise<void> => {
  try {
 ...
 */
export const importTreeDiggerState = async (
  file: File,
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const stateExport = JSON.parse(jsonString) as TreeDiggerStateExport;
 ...
 */
export const importTreeDiggerStateFromClipboard =
  async (): Promise<TreeDiggerStateExport> => {
    try {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        ...
        throw new Error(
          "Clipboard reading requires secure context (HTTPS) and user interaction",
        );
      }
      ...
      let stateExport: TreeDiggerStateExport;
      try {
        stateExport = JSON.parse(jsonString) as TreeDiggerStateExport;
      } catch (parseError) {
      ...
      log(
        `Successfully parsed clipboard state with version ${stateExport.metadata.version}`,
      );
      return stateExport;
    } catch (error) {
 ...
 */
export const formatStateFileSize = (bytes: number): string => {
 ...
 */
export const estimateStateFileSize = (
  analysis: TreeDiggerAnalysis | null,
): number => {
