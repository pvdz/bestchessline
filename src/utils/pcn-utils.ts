 ...
 */
export function convertSANLineToPCN(
  sanMoves: string[],
  startingFEN: string,
): string {
 ...
 */
export function convertPCNToSAN(pcnMove: string, _fen: string): string | null {
  try {
 ...
 * @returns SAN line
 */
export function convertPCNLineToSAN(
  pcnLine: string,
  startingFEN: string,
): string {
 ...
 */
export function extractPCNMoves(pcnLine: string): string[] {
  const moves: string[] = [];
  const moveGroups = pcnLine.split(/\d+\.\s+/).filter((group) => group.trim());
 ...
 */
function getPieceCapitalized(piece: string): string {
 ...
 */
export function formatPCNLineWithMoveNumbers(pcns: string[]): string {
  let result = "";
 ...
 */
export function computeSanGameFromPCN(
  pcns: string[],
  rootFEN: string,
  justThrow: boolean = false,
): string {
  ...
  for (const pcn of pcns) {
    ...
    if (sanMove) sanMoves.push(sanMove);

    // Derive the concrete move directly from PCN to avoid SAN parsing ambiguity
    let moveObj: ReturnType<typeof parseMove> | null = null;
    ...
    if (pcn === "O-O" || pcn === "O-O-O") {
      ...
      if (!boardPiece) {
        ...
        if (justThrow) throw new Error(msg);
        else continue;
      }
      ...
      const isCastlingLike =
        isKing && sameRank && Math.abs(toFile - fromFile) === 2;
      moveObj = {
        from,
        to,
        piece: boardPiece,
        ...(isCastlingLike
          ? {
              special: "castling",
              rookFrom:
                boardPiece === boardPiece.toUpperCase()
                  ? toFile > fromFile
                    ? "h1"
                    : "a1"
                  : toFile > fromFile
    ...
    const result = analyzeMove(currentFEN, moveObj as any);
    if (!result.isValid) {
    ...
    currentFEN = applyMoveToFEN(currentFEN, moveObj as any, { assert: false });
  }
