...
export const importFishStateFromClipboard = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      const reconstructFishLine = (
        ...
        for (const pcn of streamlinedLine.pcns) {
          ...
          if (move) {
            ...
            console.warn(
              `Failed to parse move ${pcn} in position ${currentFEN}`,
            );
      ...
      const defaultConfig: LineFisherConfig = {
 ...
 */
interface FishLine {
  lineIndex: number; // Basically a counter
 ...
 */
export async function fish(config: LineFisherConfig): Promise<FishLine[]> {
  ...
  try {
    ...
    const bestMove = rootAnalysis.moves.sort((a: any, b: any) =>
 ...
 */
async function createInitialMove(state: FishState): Promise<FishLine> {
  const { config } = state;
 ...
 */
function updateFishConfigDisplay(config: LineFisherConfig): void {
  try {
...
export const exportFishStateToClipboard = async (): Promise<void> => {
  ...
  console.log(
    "Current fish state done length:",
    currentFishState?.done?.length,
  );
...
const getRandomProofString = (): string => {
 ...
 */
const updateLineElement = (lineElement: HTMLElement, line: FishLine): void => {
...
const updateLineFisherExploredLines = (
  ...
  const headerElement = querySelectorOrThrow(
    resultsElement,
    "#line-fisher-results-header",
  );
 ...
 */
const updateLineFisherButtonStates = (): void => {
