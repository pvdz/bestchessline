...
import { getFENWithCorrectMoveCounter } from "../../utils/utils.js";
 ...
 */
export function updateFENInput(): void {
 ...
 */
export function updateControlsFromPosition(): void {
  const position = Board.getPosition();
 ...
 */
export function updatePositionFromControls(): void {
  // Get current player
  const whiteRadio = getCheckedRadio("current-player", "w");
 ...
 */
export const initializePositionEvaluationButton = (): void => {
  const evaluationButton = getElementByIdOrThrow(
 ...
 */
export const evaluateCurrentPosition = async (): Promise<void> => {
  const currentFEN = Board.getFEN();
  if (!currentFEN) {
 ...
 */
export const updatePositionEvaluationDisplay = (): void => {
  const evaluationButton = getElementByIdOrThrow(
    "position-evaluation-btn",
  ) as HTMLButtonElement;
  if (!evaluationButton) {
