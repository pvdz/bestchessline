  ...
  if (wasCorrectMove) {
    ...
    if (bestFirst) {
      ...
      if (bestFirst.move !== normalizedMoveNotation) {
        showInfoToast(
          "Note: Accepted move was not best move in position. Most likely a locked predefined opening.",
          dom,
        );
      }
    }
    ...
    if (best && found) {
      ...
      const chip = panel.querySelector(
        `.practice-score-chip[data-move="${attemptedLong}"]`,
      ) as HTMLElement | null;
      if (chip) {
  ...
  const altsHtml = altsList.length
    ? `<div class="practice-alts">
         ...
         <div class="practice-score-row">${altsList
           ...
           .map(
             (m) =>
               `<span class=\"practice-score-chip\" data-move=\"${m.move}\">${scoreToStr(m.score)}</span>`,
           )
           .join("")}</div>
       </div>`
...
// Show hint
export function showHintForCurrentPosition(
  gameState: GameState,
  dom?: DOMElements,
): void {
  if (!gameState.isPracticeActive || !gameState.isHumanTurn) return;
...
function isPieceOfCurrentPlayer(piece: string, fen: string): boolean {
...
function highlightValidMoves(moves: string[]): void {
