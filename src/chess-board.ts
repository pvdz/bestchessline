...
export class ChessBoard {
  ...
  private createPieceElement(piece: string, square: string): HTMLElement {
  ...
  private getPieceSymbol(type: PieceType, color: Color): string {
  ...
  private updateDropTarget(clientX: number, clientY: number): void {
    ...
    if (this.currentDropTarget && this.currentDropTarget !== newDropTarget) {
      const prevSquare = this.element.querySelector(
        `[data-square="${this.currentDropTarget}"]`,
      );
    ...
    if (newDropTarget && newDropTarget !== this.currentDropTarget) {
      const newSquare = this.element.querySelector(
        `[data-square="${newDropTarget}"]`,
      );
  ...
  private isValidMove(from: string, to: string, piece: string): boolean {
    ...
    if (targetColor && pieceColor === targetColor) return false;
  ...
  public clearLastMoveHighlight(): void {
    // Remove highlight classes from all squares
    const highlightedSquares = this.element.querySelectorAll(
  ...
  private positionArrow(arrow: HTMLElement, from: string, to: string): void {
    ...
    // Calculate angle
    const angle =
      (Math.atan2(toCenter.y - fromCenter.y, toCenter.x - fromCenter.x) * 180) /
      Math.PI;
