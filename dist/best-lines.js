...
import { moveToNotation, compareAnalysisMoves, getFENWithCorrectMoveCounter, getGlobalCurrentMoveIndex, applyMoveToFEN, parseSimpleMove, calculateTotalPositionsWithOverrides, getStartingPlayer, } from "./utils.js";
 ...
 */
const processInitiatorMoves = async (fen, analysis) => {
    const position = parseFEN(fen);
    ...
    if (initiatorMoveIndex < initiatorMoves.length &&
        ...
        if (result) {
            ...
            if (moveNumber === 1 && analysis.initialPositionScore === undefined) {
                try {
                    ...
                    logError(`[FIRST_MOVE_DEBUG] Failed to evaluate initial position:`, error);
                }
            }
            try {
                ...
                logError(`[FIRST_MOVE_DEBUG] Failed to evaluate predefined move ${moveText}:`, error);
            ...
            const node = createNode(fen, move, score, depth, true, moveNumber, undefined, // parent
            mateIn);
        ...
        const node = createNode(fen, bestMove.move, bestMove.score, bestMove.depth, true, moveNumber, undefined, // parent
        bestMove.mateIn);
        node.analysisResult = analysisResult;
        analysis.nodes.push(node);
        analysis.analysisQueue.push(newFen);
        log(`Applied initiator move from analysis: ${moveToNotation(bestMove.move)} (score: ${bestMove.score}) -> ${newFen}`);
    }
};
/**
 ...
 */
const processResponderMoves = async (fen, analysis) => {
    const position = parseFEN(fen);
    const moveNumber = position.fullMoveNumber;
    ...
    for (const analysisMove of topMoves) {
        ...
        // Create node for this move
        const node = createNode(fen, analysisMove.move, analysisMove.score, analysisMove.depth, false, moveNumber, undefined, // parent
        analysisMove.mateIn);
        node.analysisResult = analysisResult;
        analysis.nodes.push(node);
        analysis.analysisQueue.push(newFen);
...
const buildAnalysisTree = async (fen, analysis, parentNode, depth) => {
    ...
    window.dispatchEvent(new CustomEvent("best-lines-progress", {
        detail: {
