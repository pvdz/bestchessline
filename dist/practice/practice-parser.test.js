...
function runManualMoveTest(testCase) {
...
function runOpeningLineTest(testCase) {
  ...
  const isSuccess =
    ...
    line.moves.every((move, index) => move === testCase.expected.moves[index]);
  if (isSuccess) {
    console.log("✅ PASS");
  } else {
    console.log("❌ FAIL");
...
function runPositionBuildingTest(testCase) {
  ...
  const isSuccess =
    ...
    actualPositions.every(
      (pos, index) => pos === testCase.expectedPositions[index],
    );
  if (isSuccess) {
...
//# sourceMappingURL=practice-parser.test.js.map
