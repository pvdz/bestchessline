import { parseFEN, squareToCoords, coordsToSquare, } from "../utils/fen-utils.js";
...
export function addDragAndDropListeners(gameState, dom) {
    ...
    const handleMouseUp = (event) => {
        if (dragState.isDragging) {
            ...
            const distance = Math.sqrt(Math.pow(event.clientX - dragState.startPosition.x, 2) +
    ...
    const endDrag = (clientX, clientY, gameState, dom) => {
        if (!dragState.isDragging || !dragState.originalSquare)
            return;
        ...
        if (targetSquare && targetSquare !== dragState.originalSquare) {
            ...
            const fromPiece = getPieceAtSquareFromFEN(dragState.originalSquare, gameState.currentFEN);
            const toPiece = getPieceAtSquareFromFEN(targetSquare, gameState.currentFEN);
            if (fromPiece) {
                ...
                if (gameState.isPracticeActive &&
                    ...
                    const isValidTarget = !toPiece ||
                        isWhiteFromPiece !== (toPiece === toPiece.toUpperCase());
                    if (isValidTarget) {
                        // First, move the piece visually on the board
                        const fromSquareEl = document.querySelector(`[data-square="${dragState.originalSquare}"]`);
                        const toSquareEl = document.querySelector(`[data-square="${targetSquare}"]`);
                        if (fromSquareEl && toSquareEl) {
                            // Move the piece element
                            const pieceElement = fromSquareEl.querySelector(".practice-piece");
                            if (pieceElement) {
...
export function clearBoardSelectionWithoutLastMove() {
    // Clear selected square
    const selectedEl = document.querySelector(".practice-square.selected");
    if (selectedEl) {
        selectedEl.classList.remove("selected");
