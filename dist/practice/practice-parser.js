...
import { convertSANLineToPCN, formatPCNLineWithMoveNumbers, } from "../utils/pcn-utils.js";
...
export function convertOpeningLinesToPCN(openingLines, startingFEN) {
    return openingLines.map((line) => {
        // Check if moves are already in PCN format (contain piece letters)
        const isPCNFormat = line.moves.some((move) => move.match(/^[NBRQKP][a-h][1-8][a-h][1-8]$/));
        let pcnMoves;
        if (isPCNFormat) {
        ...
        const result = {
            ...
            moves: pcnMoves
                .split(" ")
                .filter((move) => move.length > 0 && !move.match(/^\d+\.$/)),
        };
        return result;
    });
}
