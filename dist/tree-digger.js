...
const initializeTreeDiggerAnalysis = () => {
    ...
    const totalPositions = calculateTotalPositionsWithOverrides(maxDepth, responderMovesCount, firstReplyOverride, secondReplyOverride);
    log(`Initializing tree digger analysis from current position: ${rootFen}`);
    log(`Configuration: depthScaler=${depthScaler} (maxDepth=${maxDepth}), responderMoves=${responderMovesCount}, threads=${threads}, initiatorMoves=${initiatorMoves.join(",")}, overrides=[${firstReplyOverride},${secondReplyOverride}]`);
    return {
        rootFen,
        nodes: [],
        maxDepth,
 ...
 */
const createNode = (fen, move, score, depth, isWhiteMove, moveNumber, parent, mateIn) => {
    return {
        fen,
        move,
        score,
...
export const startTreeDiggerAnalysis = async () => {
    ...
    try {
        ...
        log("Starting analysis with depth scaler:", getDepthScaler(), "-> maxDepth:", analysis.maxDepth);
...
const buildAnalysisTree = async (fen, analysis, parentNode, depth) => {
    ...
    const isInitiatorTurn = (startingPlayer === PLAYER_COLORS.WHITE && isWhiteTurn) ||
...
const processInitiatorMoveInTree = async (fen, analysis, parentNode, depth) => {
    ...
    log(`Processing initiator move at depth ${depth}, current game move: ${currentGameMoveNumber}, tree move: ${moveNumber}`);
    // Get initiator moves from predefined config
    const initiatorMoves = analysis.config.initiatorMoves;
    if (initiatorMoveIndex < initiatorMoves.length &&
        ...
        if (result) {
            ...
            const node = createNode(fen, move, score, analysisDepth, true, moveNumber, parentNode || undefined, mateIn);
            node.analysisResult = analysisResult || undefined;
            // Add to parent's children if it exists, otherwise add to root nodes
            if (parentNode) {
                parentNode.children.push(node);
                log(`Added node: ${moveToNotation(node.move)} -> parent: ${moveToNotation(parentNode.move)}`);
            }
            else {
                // Root node - add to root nodes array
                analysis.nodes.push(node);
                log(`ROOT NODE CREATED: with predefined move: ${moveToNotation(node.move)}`);
                const treeSection = document.getElementById("tree-digger-tree-empty-message");
                if (treeSection) {
                    treeSection.remove();
                }
            }
            log(`Applied initiator move from predefined config: ${moveText} -> ${newFen}`);
            // Continue building the tree from this new position
            log(`Recursive call: depth ${depth} -> ${depth + 1}, position: ${newFen.substring(0, 30)}...`);
            await buildAnalysisTree(newFen, analysis, node, depth + 1);
            return; // Successfully processed predefined move, exit
        }
        else {
            logError(`Failed to parse initiator move from predefined config: ${moveText}`);
            console.warn(`predefined move "${moveText}" failed to parse, falling back to analysis`);
    ...
    const fullyAnalyzedMoves = analysisResult.moves.filter((move) => move.depth >= targetDepth);
    ...
    const sortedMoves = fullyAnalyzedMoves.sort((a, b) => compareAnalysisMoves(a, b, direction));
    // Take the best move
    const bestMove = sortedMoves[0];
    const newFen = applyMoveToFEN(fen, bestMove.move);
    // Create node for this move
    const node = createNode(fen, bestMove.move, bestMove.score, bestMove.depth, true, moveNumber, // Use calculated move number
    parentNode || undefined, bestMove.mateIn);
    log(`Created analyzed white node with move number: ${node.moveNumber}, move: ${moveToNotation(node.move)}`);
    // Add to parent's children if it exists, otherwise add to root nodes
    if (parentNode) {
        parentNode.children.push(node);
        log(`Added node: ${moveToNotation(node.move)} -> parent: ${moveToNotation(parentNode.move)}`);
    ...
    log(`Applied initiator move from analysis: ${moveToNotation(bestMove.move)} -> ${newFen}`);
...
const processResponderMovesInTree = async (fen, analysis, parentNode, depth) => {
    ...
    log(`Analyzing responder responses at depth ${depth}, current game move: ${currentGameMoveNumber}, tree move: ${moveNumber}, position: ${fen.substring(0, 30)}...`);
    ...
    log(`Analysis complete: ${analysisResult.moves.length} moves found, depths: ${analysisResult.moves.map((m) => m.depth).join(", ")}`);
    // Increment PV lines counter
    incrementPVLines();
    // Dispatch analysis complete event
    window.dispatchEvent(new CustomEvent("stockfish-analysis-complete", {
        detail: {
    ...
    const targetDepth = getAnalysisOptions(analysis).depth || 20;
    const fullyAnalyzedMoves = analysisResult.moves.filter((move) => move.depth >= targetDepth);
    log(`Moves at target depth ${targetDepth}: ${fullyAnalyzedMoves.length}/${analysisResult.moves.length}`);
    if (fullyAnalyzedMoves.length === 0) {
    ...
    log(`Selected ${topMoves.length} moves (override: ${responderMovesToAnalyze}): ${topMoves.map((m) => `${moveToNotation(m.move)} (${m.score})`).join(", ")}`);
    ...
    for (const analysisMove of topMoves) {
        ...
        if (analysis.analyzedPositions.has(newFen)) {
            log(`Transposition detected: ${moveToNotation(analysisMove.move)} -> ${newFen} (already analyzed)`);
            continue;
        }
        // Create new node for this move
        const node = createNode(fen, analysisMove.move, analysisMove.score, analysisMove.depth, false, moveNumber, parentNode || undefined, analysisMove.mateIn);
        log(`Created responder node with move number: ${node.moveNumber}, move: ${moveToNotation(node.move)}`);
        node.analysisResult = analysisResult;
        // Add to parent's children if it exists, otherwise add to root nodes
        if (parentNode) {
            parentNode.children.push(node);
            log(`Added responder node: ${moveToNotation(node.move)} -> parent: ${moveToNotation(parentNode.move)}`);
        }
        else {
            // Root node - add to root nodes array
            analysis.nodes.push(node);
        ...
        log(`Found responder response: ${moveToNotation(analysisMove.move)} (score: ${analysisMove.score}) -> ${newFen}`);
 ...
 */
export const restoreTreeDiggerState = (importedState) => {
 ...
 */
const processAnalysisQueue = async (analysis) => {
    log(`Processing analysis queue with ${analysis.analysisQueue.length} positions...`);
    ...
    if (analysis.analysisQueue.length === 0) {
        ...
        log(`Analysis paused with ${analysis.analysisQueue.length} positions remaining`);
    }
};
/**
 * Find a node by FEN position
 */
const findNodeByFen = (nodes, fen) => {
 ...
 */
const addNewPositionsToQueue = async (analysis, node, result) => {
 ...
 */
const shouldAnalyzePosition = (analysis, fen, depth) => {
        ...
        for (const node of nodes) {
            ...
            if (node.analysisResult &&
                ...
                if (!analysis.analyzedPositions.has(fen)) {
                    ...
                    analysis.analyzedPositions.add(fen);
                    log(`Added analyzed node to queue for expansion: ${fen} (depth ${node.depth})`);
                }
            }
 ...
 */
export const continueTreeDiggerAnalysis = async () => {
    ...
    log(`Current analysis state: queue=${currentAnalysis.analysisQueue.length}, isComplete=${currentAnalysis.isComplete}, nodes=${currentAnalysis.nodes.length}`);
    ...
    if (currentAnalysis.config.threads !== newThreadCount) {
        log(`Updating thread count from ${currentAnalysis.config.threads} to ${newThreadCount}`);
    ...
    if (currentAnalysis.analysisQueue.length === 0) {
        ...
        log(`After rebuild: queue=${currentAnalysis.analysisQueue.length}, isComplete=${currentAnalysis.isComplete}`);
    }
 ...
 */
export const calculateUniquePositions = (_nodes, analysis) => {
    return analysis.analyzedPositions.size;
};
//# sourceMappingURL=tree-digger.js.map