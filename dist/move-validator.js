import { createPieceTypeNotation, createPieceNotation, getPieceTypeFromNotation, getColorFromNotation, } from "./types.js";
 ...
 */
function isLegalKnightMove(position, move) {
    const [fromRank, fromFile] = squareToCoords(move.from);
    const [toRank, toFile] = squareToCoords(move.to);
 ...
 */
function isLegalKingMove(position, move) {
    const [fromRank, fromFile] = squareToCoords(move.from);
    const [toRank, toFile] = squareToCoords(move.to);
    const rankDiff = Math.abs(toRank - fromRank);
    const fileDiff = Math.abs(toFile - fromFile);
