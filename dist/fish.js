...
export const importFishStateFromClipboard = async () => {
  ...
  try {
    ...
    const reconstructedWip = importedState.wip.map((line) =>
      reconstructFishLine(line, false),
    );
    const reconstructedDone = importedState.done.map((line) =>
 ...
 */
function convertFishConfigToLineFisherConfig(config) {
  return {
...
function updateFishButtonStates(isAnalyzing) {
  ...
  window.isFishAnalysisRunning = isAnalyzing;
 ...
 */
function formatMovesWithNumbers(sans) {
  let result = "";
 ...
 */
function convertFishLineToDisplayFormat(line) {
  return {
    sans: line.sans,
 ...
 * Non-blocking UI update with error handling
 */
function updateFishLineNode(line, state) {
  try {
    ...
    if (existingLineElement) {
      ...
      if (lineNumberElement) {
        ...
        const lineNumberHTMLElement = lineNumberElement;
        if (!line.isDone) {
