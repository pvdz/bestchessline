export async function apiLineGet(position, // FEN
...
export async function apiLinesPut(rootFEN, moves, // long moves. set to null when this is unknown. this is used to update the server for the practice app.
nowFEN, best, searchLineCount, maxDepth) {
    const obj = {
        root: rootFEN,
        moves,
        position: nowFEN,
            ...
            switch (key) {
                case "root": {
                    if (typeof obj.root !== "string") {
                        return console.log("validateServerLine(): rejected: root is not a string", obj.root);
                    }
                    break;
                }
                case "moves": {
                    ...
                    if (obj.moves.some((move) => !/^[a-h][1-8][a-h][1-8]$/.test(move)))
                        return console.log("moves is not an array of long move strings of length 4", obj.moves);
                    break;
                }
                case "position": {
                    if (typeof obj.position !== "string") {
                        return console.log("validateServerLine(): rejected: position is not a string", obj.position);
                    }
                    break;
                }
                case "bestMoves": {
                    if (!Array.isArray(obj.bestMoves)) {
                        return console.log("validateServerLine(): rejected: bestMoves is not an array", obj.bestMoves);
                    }
                    if (obj.bestMoves.some((move) => {
                        ...
                        return console.log("validateServerLine(): rejected: bestMoves is not an array of objects with score, move, mateIn, and draw", obj.bestMoves);
                    }
                    break;
                }
                case "searchLineCount": {
                    if (typeof obj.searchLineCount !== "number") {
                        return console.log("validateServerLine(): rejected: searchLineCount is not a number", obj.searchLineCount);
                    }
                    break;
                }
                case "maxDepth": {
                    if (typeof obj.maxDepth !== "number")
                        return console.log("validateServerLine(): rejected: maxDepth is not a number", obj.maxDepth);
                    break;
                }
                default: {
