...
export function updateFishPvTickerThrottled(moves, fen, force = false) {
  ...
  const sortedAll = lastMoves.slice(0).sort((a, b) => {
...
export function updateLiveLinesPreview() {
  ...
  try {
    ...
    const wipHeading =
      document.querySelector("#fish-lines-wip")?.previousElementSibling;
    const doneHeading =
      document.querySelector("#fish-lines-done")?.previousElementSibling;
    if (wipHeading) {
    ...
    const renderLines = (container, lines) => {
      ...
      const html = lines
        .map(
          (l) =>
            `<div class="fish-live-line" data-line-id="${l.lineIndex}">${formatPCNLineWithMoveNumbers(l.pcns)}</div>`,
        )
        .join("");
