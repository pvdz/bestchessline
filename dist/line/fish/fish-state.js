...
let currentFishState = createFishState(defaultConfig());
function createFishState(config) {
 ...
 */
export const importFishState = (importedState) => {
    try {
        ...
        const reconstructFishLine = (streamlinedLine, isFromDoneArray, index) => {
        ...
        const reconstructedWip = importedState.wip.map((line, index) => reconstructFishLine(line, false, index));
        const reconstructedDone = importedState.done.map((line, index) => reconstructFishLine(line, true, index));
...
//# sourceMappingURL=fish-state.js.map