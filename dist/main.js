import { moveToNotation, parseFEN, toFEN, squareToCoords, coordsToSquare, log, logError, getInputElement, getTextAreaElement, getButtonElement, getCheckedRadio, getCheckedRadioByName, querySelectorHTMLElementBySelector, getFENWithCorrectMoveCounter, setGlobalCurrentMoveIndex, } from "./utils.js";
...
const initializeEventListeners = () => {
    ...
    [whiteMovesInput, blackMovesInput, maxDepthInput, threadsInput].forEach((input) => {
        if (input) {
    ...
    const treeDiggerBlackMovesInput = document.getElementById("tree-digger-black-moves");
...
const getBestLinesThreadCount = () => {
    const threadsInput = document.getElementById("tree-digger-threads");
    return threadsInput ? (parseInt(threadsInput.value) ?? 4) : 10;
};
/**
      ...
      <div class="best-line-progress-left">
        ...
        <div class="best-line-settings">
          ...
          <div class="setting">
            ...
            <div class="setting-value">${getBestLinesThreadCount()}</div>
          </div>
        </div>
...
const findFromSquareWithDisambiguation = (piece, toSquare, disambiguation, currentFEN) => {
    ...
    if (disambiguation) {
        const filtered = candidates.filter((square) => square.includes(disambiguation[0]) ||
...
const clearWhiteMoveInputs = () => {
    ...
    if (whiteMove1Input)
        whiteMove1Input.value = "";
    if (whiteMove2Input)
        whiteMove2Input.value = "";
};
/**
