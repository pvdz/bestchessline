...
const initializeEventListeners = () => {
  ...
  const stateFileInput = getElementByIdOrThrow("tree-digger-state-file");
  ...
  const debouncedTreeDiggerUpdate = () => {
    ...
    if (treeDiggerUpdateTimeout) return;
...
async function runFish2Analysis() {
  try {
    ...
    const fenInput = getElementByIdOrThrow("fen-input");
    const currentFEN =
      fenInput?.value ||
      "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
    // Get analysis options from UI
    const depthInput = getElementByIdOrThrow("line-fisher-depth");
    const threadsInput = getElementByIdOrThrow("line-fisher-threads");
