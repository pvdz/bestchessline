...
function runTest() {
    ...
    console.log(`Parsed move: ${parsedMove.from} → ${parsedMove.to} (${parsedMove.piece})`);
    // Check if the parsed move matches expected
    const isParseSuccess = parsedMove.from === testCase.expected.from &&
    ...
    if (originalTurn === "b" && resultTurn === "w") {
        ...
        console.log(`❌ FAIL: Turn not changed correctly. Original: ${originalTurn}, Result: ${resultTurn}`);
        return false;
    }
...
runAllTests();
//# sourceMappingURL=fen-manipulation.test.js.map