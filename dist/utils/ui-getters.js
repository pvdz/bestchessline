...
export function getDepthScaler() {
    const depthScalerInput = getElementByIdOrThrow("tree-digger-depth-scaler");
    const depthScalerValue = getElementByIdOrThrow("tree-digger-depth-scaler-value");
...
export function getResponderMovesCount() {
    const responderMovesInput = getElementByIdOrThrow("tree-digger-responder-moves");
    const responderMovesValue = getElementByIdOrThrow("tree-digger-responder-moves-value");
    // Try to get the value from the display span first, then fall back to input value
...
export function getThreadCount() {
    const threadsInput = getElementByIdOrThrow("tree-digger-threads");
...
export function getFirstReplyOverride() {
    const overrideInput = getElementByIdOrThrow("tree-digger-override-1");
...
export function getSecondReplyOverride() {
    const overrideInput = getElementByIdOrThrow("tree-digger-override-2");
