...
import { getPieceTypeFromNotation, getColorFromNotation, createPieceNotation, } from "../line/types.js";
...
export function parseSimpleMove(moveText, fen) {
    ...
    if (cleanMove.match(/^[a-h]x[a-h][1-8]$/)) {
        ...
        const validCandidates = candidates.filter((fromSquare) => canPawnMoveTo(fromSquare, toSquare, position.board));
        // Use the file information to disambiguate
        const fileCandidates = validCandidates.filter((fromSquare) => fromSquare[0] === fromFile);
 ...
 */
export function parseLongMove(moveStr, fen) {
    if (moveStr.length !== 4)
        return null;
 ...
 */
export function parseMoveFromNotation(moveText, isWhiteTurn = true) {
    ...
    if (cleanMove.match(/^[a-h]x[a-h][1-8]$/)) {
        ...
        const toSquare = cleanMove.substring(2);
        const piece = (isWhiteTurn ? "P" : "p");
        const fromSquare = inferPawnFromSquareForCapture(fromFile, toSquare, isWhiteTurn);
        if (fromSquare) {
            return { from: fromSquare, to: toSquare, piece };
        }
    }
    ...
    if (pieceMatch) {
        ...
        const toSquare = pieceMatch[3];
        const pieceNotation = (isWhiteTurn ? pieceType : pieceType.toLowerCase());
        const fromSquare = inferPieceFromSquareWithDisambiguation(pieceNotation, toSquare, disambiguation, isWhiteTurn);
        if (fromSquare) {
            const piece = (isWhiteTurn ? pieceType : pieceType.toLowerCase());
            return { from: fromSquare, to: toSquare, piece };
        }
...
export function findFromSquareWithDisambiguation(piece, toSquare, disambiguation, currentFEN) {
    ...
    if (validCandidates.length > 1) {
        ...
        return selectCorrectMove(validCandidates, toSquare, piece, position.board, disambiguation);
    }
    return null;
}
/**
 ...
 */
export function canKnightMoveTo(fromSquare, toSquare, _board) {
    const [fromRank, fromFile] = squareToCoords(fromSquare);
    const [toRank, toFile] = squareToCoords(toSquare);
 ...
 */
export function canKingMoveTo(fromSquare, toSquare, _board) {
    const [fromRank, fromFile] = squareToCoords(fromSquare);
    const [toRank, toFile] = squareToCoords(toSquare);
    const rankDiff = Math.abs(fromRank - toRank);
    const fileDiff = Math.abs(fromFile - toFile);
 ...
 */
export function selectCorrectMove(candidates, _toSquare, _piece, _board, disambiguation) {
    ...
    if (disambiguation.length === 1) {
        ...
        const exactCandidates = candidates.filter((square) => square.toLowerCase() === disambiguationSquare);
 ...
 */
function inferPieceFromSquareWithDisambiguation(piece, toSquare, disambiguation, isWhiteTurn) {
    const pieceType = getPieceTypeFromNotation(piece);
    // Handle different types of disambiguation
    if (disambiguation.length === 1) {
        // Single character disambiguation (file or rank)
...
//# sourceMappingURL=move-parser.js.map