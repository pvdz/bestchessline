...
export function parseMove(moveText, currentFEN) {
    ...
    if (moveText.match(/^[a-h]x[a-h][1-8]$/)) {
        ...
        // Try both colors to handle ambiguous cases
        let fromSquare = findFromSquareWithDisambiguation(PIECES.WHITE_PAWN, toSquare, fromFile, currentFEN);
        let piece = PIECES.WHITE_PAWN;
        if (!fromSquare) {
            fromSquare = findFromSquareWithDisambiguation(PIECES.BLACK_PAWN, toSquare, fromFile, currentFEN);
            piece = PIECES.BLACK_PAWN;
        }
        if (fromSquare) {
            return { from: fromSquare, to: toSquare, piece };
 ...
 */
export function parseGameNotation(notation, initialFEN) {
    // Clean the notation
...
//# sourceMappingURL=move-parsing.js.map