...
import { getLineFisherConfigFromUI, validateLineFisherConfig, showLineFisherConfigError, } from "./line-fisher-ui-utils.js";
import { getLineFisherState, updateLineFisherState, startLineFisherAnalysis, stopLineFisherAnalysis, resetLineFisherAnalysis, continueLineFisherAnalysis, } from "../line_fisher.js";
import { updateLineFisherConfigDisplay, updateLineFisherProgressDisplay, updateLineFisherExploredLines, } from "./line-fisher-results.js";
...
export const resetLineFisherAnalysisFromManager = async () => {
    ...
    try {
      ...
      `;
        ...
        if (progressElement) {
            const progressBar = progressElement.querySelector(".line-fisher-progress-bar-fill");
            if (progressBar) {
                progressBar.style.width = "0%";
            }
            const progressText = progressElement.querySelector(".line-fisher-progress-text");
            if (progressText) {
        ...
        if (activityElement) {
            const eventsPerSecond = activityElement.querySelector(".line-fisher-events-per-second");
            const totalEvents = activityElement.querySelector(".line-fisher-total-events");
            const activityStatus = activityElement.querySelector(".line-fisher-activity-status");
            if (eventsPerSecond)
                eventsPerSecond.textContent = "0.00";
            if (totalEvents)
 ...
 */
export const importLineFisherStateFromClipboardFromManager = async () => {
...
export const updateLineFisherButtonStates = () => {
    ...
    const startBtn = getElementByIdOrThrow("start-line-fisher");
...
export const handleLineFisherError = (error, context) => {
    ...
    const errorMessages = {
        ...
        Analysis: "Analysis encountered an error. You can try continuing or resetting.",
    ...
    const userMessage = errorMessages[category] ||
...
export const recoverFromLineFisherError = async () => {
    ...
    try {
        ...
        const configElement = document.getElementById("line-fisher-config");
        if (configElement) {
            configElement.classList.remove("line-fisher-analyzing", "line-fisher-error");
        }
        // Update button states
        updateLineFisherButtonStates();
        // Update status
        ...
        showToast("Recovered from error. You can continue or reset.", "#FF9800", 4000);
...
export const validateLineFisherState = (state) => {
    ...
    if (!state.analyzedPositions)
        errors.push("Missing analyzed positions");
    ...
    if (state.config) {
        ...
        if (typeof state.config.maxDepth !== "number")
