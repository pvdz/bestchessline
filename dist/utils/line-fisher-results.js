...
import { calculateTotalNodes, calculateTotalLines, calculateResponderNodes, } from "./line-fisher-calculations.js";
...
export const updateLineFisherActivityMonitor = (progress) => {
    ...
    if (!activityElement)
        return;
    // Update time elapsed
    const timeElapsed = progress.startTime > 0 ? (Date.now() - progress.startTime) / 1000 : 0;
    const timeElapsedElement = activityElement.querySelector(".line-fisher-time-elapsed");
...
export const updateLineFisherExploredLines = (results) => {
    ...
    for (let i = 0; i < displayCount; i++) {
        ...
        const finalScore = result.scores.length > 0 ? result.scores[result.scores.length - 1] : 0;
        ...
        const scoreText = result.scores.length === 0
            ? "?"
            : scoreInPawns > 0
                ? `+${scoreInPawns.toFixed(1)}`
                : scoreInPawns.toFixed(1);
        // Show "?" if no scores yet, otherwise show the delta
        const deltaText = result.scores.length === 0
            ? "?"
...
const addLineClickHandlers = (results) => {
    const lineElements = document.querySelectorAll(".line-fisher-result-compact[data-line-index]");
