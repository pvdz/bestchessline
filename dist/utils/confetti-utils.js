 ...
 */
function animateConfettiParticle(particle, launchPoint) {
 ...
 */
export function triggerConfetti(particleCount = 100) {
    try {
            ...
            for (let i = 0; i < particlesPerLaunch; i++) {
                setTimeout(() => {
                    try {
                        ...
                        console.error(`Error creating particle ${i + 1} from launch ${launchIndex + 1}:`, error);
                    }
                }, launchIndex * 100 + i * 30); // Stagger launches and particles
            }
        });
        ...
        setTimeout(() => {
            for (let i = 0; i < particleCount / 4; i++) {
                setTimeout(() => {
                    try {
                        const particle = createConfettiParticle();
                        const randomLaunch = launchPoints[Math.floor(Math.random() * launchPoints.length)];
                        document.body.appendChild(particle.element);
 ...
 */
export function triggerRainbowBurst() {
...
//# sourceMappingURL=confetti-utils.js.map