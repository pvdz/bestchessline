...
import { updateTreeDiggerResults, resetTreeDiggerPaginationState, } from "./tree-digger-results.js";
...
import { startTreeDiggerAnalysis, stopTreeDiggerAnalysis, clearTreeDiggerAnalysis, continueTreeDiggerAnalysis, restoreTreeDiggerState, isAnalyzing, getCurrentAnalysis, getProgress, } from "../tree-digger.js";
import { initializeStockfish } from "../stockfish-client.js";
import { updateButtonStates } from "./analysis-config.js";
import { exportTreeDiggerState, copyTreeDiggerStateToClipboard, importTreeDiggerState, importTreeDiggerStateFromClipboard, deserializeTreeDiggerState, validateTreeDiggerState, formatStateFileSize, estimateStateFileSize, } from "./tree-digger-persistence.js";
...
const showTreeDiggerInfoBoxes = () => {
    ...
    if (stateInfoElement)
        stateInfoElement.style.display = "block";
};
/**
...
export const importTreeDiggerStateFromManager = async (file) => {
    try {
        ...
        if (validation.warnings.length > 0) {
            updateTreeDiggerStatus(`State imported with warnings: ${validation.warnings.join(", ")}`);
        }
        else {
...
export const importTreeDiggerStateFromClipboardFromManager = async () => {
    try {
        ...
        updateTreeDiggerStateInfo(stateExport, validation);
        if (validation.warnings.length > 0) {
            updateTreeDiggerStatus(`State pasted with warnings: ${validation.warnings.join(", ")}`);
        }
        else {
