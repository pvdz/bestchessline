...
function testMoveParsing(move, fen, expectedFrom, expectedTo, expectedPiece, testName) {
    ...
    const fromOk = assert(result.from === expectedFrom, `${testName} - Expected from "${expectedFrom}", got "${result.from}"`);
    const toOk = assert(result.to === expectedTo, `${testName} - Expected to "${expectedTo}", got "${result.to}"`);
    const pieceOk = assert(result.piece === expectedPiece, `${testName} - Expected piece "${expectedPiece}", got "${result.piece}"`);
...
function runTests() {
    ...
    function testPositionAwareShouldFail(move, fen, name) {
        const result = parseMove(move, fen);
        return assert(result === null, `${name} - Expected failure to parse, but got ${JSON.stringify(result)}`);
    }
    let passed = 0;
    let failed = 0;
    for (const test of tests) {
        const success = testMoveParsing(test.move, test.fen, test.expectedFrom, test.expectedTo, test.expectedPiece, test.name);
        if (success) {
            passed++;
        }
        else {
    ...
    const reportedFen = "rnbqkbnr/p1pppppp/1p6/8/8/5NP1/PPPPPP1P/RNBQKB1R b KQkq - 0 1";
    if (testPositionAwareShouldFail("g3", reportedFen, "Reported case: black to move 'g3' invalid")) {
        passed++;
    }
    else {
        failed++;
...
runTests();
export { runTests };
//# sourceMappingURL=move-parser.test.js.map